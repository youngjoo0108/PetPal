...
- make_map/load_map
  ...
  - 저장한 맵의 시작 점의 좌표가 load할 때마다 달라져서 map과 odom 값이 다른 이슈
    &rarr; map.txt 마지막 줄에 x,y offset 값 추가하여 저장/로드하여 해결
...
_Node Name : patrol_route.py_

- 700X700 격자를 다시 20X20의 큰 격자로 나누어 빈 공간 여부 판단
- 빈 공간 좌표 배열로 모아 최적의 순찰 루트 결정
